...
mean <- smry[4]
```
So the mean total steps taken per day is `r mean`, and the median of the total steps taken per day is `r median`.
...
We note that only the steps column contains any (as expected), with `r nanum[1]` total NA values.
...
imean <- smry[4]
```
So the mean total steps taken per day is `r imean`, and the median of the total steps taken per day is `r imedian`. These differ from before by `r abs(imean-mean)` for the mean, and `r abs(imedian-median)` for the median. Note the minimal difference in the median but not the mean, as would be expected mathematically for this particular procedure.
